...
{
    console.error("未获取读取屏幕权限,脚本无法正常运行,退出.....")
    exit();
}
toast("按音量下键停止脚本")
console.info("验证卡密中")
...
var target_num =  Number(args.target_num);
var gender_choose = Number(args.gender_choose);
...
let pjysdk = new PJYSDK("brcavlco6itbb1g4bnqg", "KSZ9zTvNQaRviJkD7w2IFZK7tiYD46VG"); 
pjysdk.debug = false;
...
function main()
{
    ...
    {
        console.error("获取访客列表失败,脚本退出,请反馈bug")
    }
}
function chat()
{
...
function get_name()
{
    ...
    log("点击\"我的\"页面,如果是大神请手动切换下")
    text("我的").findOne().parent().parent().click();
    sleep(1000)
    text("最近来访").findOne().parent().click()
    sleep(1000)
    while(user_name_arr.length<=target_num)
    {
        sleep(2000)
        var user_name = id("user_name").find();
        console.info("访客列表获取成功")
        // back();
        //log(user_name.length)
        for(let i=6;i<user_name.length;i++)
        {
            user_name_arr.push(user_name[i].text()) 
...
function search(name)
{
    ...
    sleep(500)
    id("eTSearch").findOne().setText(name)
    sleep(1000)
    text("搜索").findOne().click()
    sleep(2000)
    id("mViewUserAvatar").findOne().parent().parent().click()
    sleep(500)
    if(is_customer())
    {
...
{
        while(!id("name").exists())
    {
        sleep(1000)
    }
    log(id("name").findOne().getText()+"--判断是否存在技能")
    if(text("技能").exists())
    {
        log("存在技能,放弃")
        return false;
    ...
    console.show();
    if(point){
        log("判断"+id("name").findOne().getText()+"为雄性")
        if(gender_choose==0)return true;
        else return false;
    }else{
        log("判断"+id("name").findOne().getText()+"为雌性")
        if(gender_choose==0)return false;
        else return false
    }
}